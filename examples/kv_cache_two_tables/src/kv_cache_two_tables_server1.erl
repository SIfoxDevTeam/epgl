-module(kv_cache_two_tables_server1).
...
-define(TABLE_NAME, "public.test_table1").
-record(state, {}).
...
process_row(#row{table_name = ?TABLE_NAME, change_type = delete, fields = [Id, _Value]}) ->
    io:format("SRV1: delete key from ETS ~p ~n", [Id]),
    ets:delete(?MODULE, Id);
process_row(#row{table_name = ?TABLE_NAME, fields = [Id, Value]}) ->
    io:format("SRV1: insert/update row to ETS ~p ~n", [{Id, Value}]),
